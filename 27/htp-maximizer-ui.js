...
(function(W) {
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      .htp-maximizer-panel h3::before { content: '⚡'; }
      .htp-bet-input-row { display: flex; align-items: center; gap: 10px; margin-bottom: 16px; }
      .htp-bet-input {
        flex: 1; background: #1e293b;
        border: 1px solid rgba(73,232,194,0.3); border-radius: 8px;
        padding: 10px 14px; color: #fff; font-size: 16px; font-weight: 600;
        outline: none; transition: border-color 0.2s;
      }
      .htp-bet-input:focus { border-color: #49e8c2; }
      .htp-bet-input::placeholder { color: #475569; }
      .htp-kas-label { color: #49e8c2; font-weight: 700; font-size: 14px; min-width: 36px; }
      .htp-split-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 14px; }
      .htp-split-box { background: #1e293b; border-radius: 8px; padding: 12px; text-align: center; }
      .htp-split-box .label { font-size: 11px; color: #64748b; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 4px; }
      .htp-split-box .value { font-size: 18px; font-weight: 700; color: #fff; }
      .htp-split-box.pool .value  { color: #49e8c2; }
      .htp-split-box.hedge .value { color: #f59e0b; }
      .htp-cap-bar-wrap { margin-bottom: 14px; }
      .htp-cap-bar-label { display: flex; justify-content: space-between; font-size: 11px; color: #64748b; margin-bottom: 5px; }
      .htp-cap-bar-track { background: #1e293b; border-radius: 99px; height: 6px; overflow: hidden; }
      .htp-cap-bar-fill { height: 100%; border-radius: 99px; background: linear-gradient(90deg, #49e8c2, #3b82f6); transition: width 0.3s ease; }
      .htp-cap-bar-fill.over { background: linear-gradient(90deg, #ef4444, #f59e0b); }
      .htp-maximizer-status { font-size: 12px; padding: 8px 12px; border-radius: 8px; margin-bottom: 14px; font-weight: 500; }
      ...
      .htp-payout-preview { background: #1e293b; border-radius: 8px; padding: 12px 14px; font-size: 12px; color: #94a3b8; margin-bottom: 14px; line-height: 1.8; }
      ...
      .htp-maximizer-toggle { display: flex; align-items: center; gap: 10px; margin-bottom: 14px; cursor: pointer; user-select: none; }
      .htp-toggle-switch { width: 40px; height: 22px; background: #334155; border-radius: 99px; position: relative; transition: background 0.2s; flex-shrink: 0; }
      .htp-toggle-switch.on { background: #49e8c2; }
      .htp-toggle-switch::after { content: ''; position: absolute; width: 16px; height: 16px; background: #fff; border-radius: 50%; top: 3px; left: 3px; transition: transform 0.2s; }
      ...
      .htp-place-btn { width: 100%; padding: 12px; background: linear-gradient(135deg, #49e8c2, #3b82f6); color: #0f172a; font-weight: 700; font-size: 14px; border: none; border-radius: 8px; cursor: pointer; transition: opacity 0.2s, transform 0.1s; letter-spacing: 0.03em; }
  ...
  function render(containerId, eventConfig) {
    ...
    container.innerHTML = `
      <div class="htp-maximizer-panel">
        ...
        <div class="htp-split-grid" id="htp-mx-split" style="display:none">
          <div class="htp-split-box pool"><div class="label">→ Pool</div><div class="value" id="htp-mx-pool">0 KAS</div></div>
          <div class="htp-split-box hedge"><div class="label">→ Hedge</div><div class="value" id="htp-mx-hedge">0 KAS</div></div>
        </div>
        <div class="htp-cap-bar-wrap" id="htp-mx-capwrap" style="display:none">
          <div class="htp-cap-bar-label"><span>Maximizer capacity</span><span id="htp-mx-cap-text">— / —</span></div>
          <div class="htp-cap-bar-track"><div class="htp-cap-bar-fill" id="htp-mx-capfill" style="width:0%"></div></div>
        </div>
    ...
    function update() {
      ...
      if (bet <= 0) {
        splitEl.style.display = 'none'; capWrap.style.display = 'none';
        statusEl.style.display = 'none'; previewEl.style.display = 'none';
        btn.disabled = true; btn.textContent = 'Enter amount to continue';
        return;
      }

      if (!isMaximizer) {
        splitEl.style.display = 'none'; capWrap.style.display = 'none'; statusEl.style.display = 'none';
        const gross = bet * cfg.currentOdds;
        const fee   = (gross - bet) * 0.02;
        previewEl.style.display = 'block';
        previewEl.innerHTML = `<span class="win">WIN</span>: ${(gross - fee).toFixed(2)} KAS &nbsp;<span class="fee">(2% fee: ${fee.toFixed(2)} KAS)</span><br><span class="lose">LOSE</span>: 0 KAS`;
        btn.disabled = false; btn.textContent = `Place ${bet} KAS Standard Bet`;
        return;
      }

      const Fee = getFee();
      if (!Fee) {
        // Fee shim not yet loaded — show loading state, retry in 500ms
        btn.textContent = 'Loading fee engine...';
        btn.disabled = true;
      ...
      const pct  = cap > 0 ? Math.min(100, ((check.used || 0) / cap) * 100) : 0;
      ...
      previewEl.innerHTML = `
        ...
        <span class="lose">LOSE</span>: claim <strong style="color:#f59e0b">${loseCalc.claimable.toFixed(2)} KAS</strong> hedge &nbsp;<span class="fee">(30% fee: ${loseCalc.protocolFee.toFixed(2)} KAS)</span>
      `;

      btn.disabled    = !check.allowed;
      btn.textContent = check.allowed ? `Place ${bet} KAS Maximizer Bet` : 'Maximizer Cap Reached';
    }
    ...
    btn.addEventListener('click', function() {
      ...
      window.dispatchEvent(new CustomEvent('htp:bet:submit', {
        detail: {
          ...
          betKas:      bet,
          isMaximizer: isMaximizer,
          split:       isMaximizer && Fee ? Fee.maximizerSplit(bet) : null,
        }
      }));
    });
  ...
  console.log('[HTPMaximizerUI v2] loaded — uses htp-fee-shim.js');
})(window);
