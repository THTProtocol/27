...
class SettlementEngine {
  constructor(txBuilder, rpc, db, indexer) {
  ...
  async settleGame(gameId, winnerId, oracleSigs) {
    ...
    if (!escrowUtxo) throw new Error('Escrow UTXO not found for game ' + gameId);

    const potSompi = parseInt(escrowUtxo.utxoEntry.amount);
    const protocolAddr = this.db.getProtocolAddress();

    let tx;
    if (winnerId === 'draw') {
      tx = this.txBuilder.buildGameDrawTx({
        escrowUtxo, playerAAddr: game.playerA, playerBAddr: game.playerB,
        protocolAddr, stakeSompi: game.stakeSompi,
      });
    } else {
      const winnerAddr = winnerId === game.playerA ? game.playerA : game.playerB;
      tx = this.txBuilder.buildGameSettleTx({
        escrowUtxo, winnerAddr, protocolAddr, potSompi,
      });
    }
    ...
    const winnerUser = this.db.getOrCreateUser(winnerId === 'draw' ? game.playerA : winnerId);
    this.db.updateUser(winnerUser.addr, { gamesWon: winnerUser.gamesWon + (winnerId === 'draw' ? 0 : 1) });

    console.log('[SETTLEMENT] Game', gameId, 'settled. TX:', txId);
    return { txId };
  }
