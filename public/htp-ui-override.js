...
function init() {
  // Kill all legacy SPA view divs
  ['v-overview','v-markets','v-create','v-skill','v-oracle',
   'v-portfolio','v-wallet','v-kaspa','v-lobby','v-game',
   'v-terms','v-status','v-admin','v-docs','v-tx',
  ...
  // Ensure htp-root exists and is visible
  ...
  // Override go() to delegate to hash router
  ...
  window.htpRouter.screenWallet = function() {
    ...
    if (addr) {
      root.innerHTML =
        ...
        '<span style="width:6px;height:6px;border-radius:50%;background:#00ff87;display:inline-block"></span>CONNECTED</span></div>' +
        ...
        '<button class="htp-btn htp-btn-ghost" style="width:100%;padding:12px;font-size:12px" ' +
        'onclick="localStorage.removeItem(\'htp_mnemonic_session\');localStorage.removeItem(\'htp_session\');window.connectedAddress=null;window.htpAddress=null;window.location.reload()">DISCONNECT</button>' +
        '</div></div>';
      ...
      root.innerHTML =
        ...
        'style="width:100%;background:rgba(0,0,0,0.4);border:1px solid var(--htp-border,#2a2a2a);border-radius:6px;padding:12px;color:var(--htp-text,#fff);font-size:13px;font-family:monospace;resize:vertical;box-sizing:border-box;outline:none" ' +
        'onfocus="this.style.borderColor=\'rgba(201,168,76,0.5)\'" ' +
        'onblur="this.style.borderColor=\'\'"></textarea>' +
        '<div id="wallet-status" style="font-size:11px;color:var(--htp-muted,#888);min-height:18px;margin:10px 0 12px"></div>' +
        '<button class="htp-btn" style="width:100%;padding:13px;font-size:13px" onclick="window.htpWalletV3.importMnemonic()">IMPORT MNEMONIC</button>' +
        '</div>' +
        '<div style="background:var(--htp-card,#181818);border:1px solid var(--htp-border,#2a2a2a);border-radius:12px;padding:24px">' +
        ...
        '<button class="htp-btn htp-btn-ghost" style="width:100%;padding:13px;font-size:13px" onclick="window.htpWalletV3.generateWallet()">GENERATE WALLET</button>' +
        '</div></div>';
    }
  };
}
