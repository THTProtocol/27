/**
 * htp-wallet-logos.js — v7.2
 * - Replaces ONLY the .w-grid element with the new card grid
 * - Preserves all existing .w-sec sections (mnemonic, hex key, custom node, etc.)
 * - Extension wallets call native selWallet() which handles TN12 mnemonic signing
 * - Mobile/hardware wallets get a kaspa: address paste input
 */
...
(function applyNavFix() {
  function fix() {
    document.querySelectorAll('.nav-btn').forEach(function(btn) {
      ...
      after.className = 'nav-active-bar';
      after.style.cssText = 'position:absolute;left:10px;right:10px;bottom:6px;height:2px;border-radius:2px;background:rgba(73,232,194,.95);box-shadow:0 0 10px rgba(73,232,194,.24);pointer-events:none';
...
// ── Wallet grid — replaces ONLY .w-grid, preserves all .w-sec sections ────────
(function patchWalletGrid() {

  function logoEl(id) {
    var url = _LOGO_URLS[id];
    if (url) {
      return '<img src="' + url + '" width="48" height="48" style="border-radius:10px;display:block;margin:0 auto 10px" loading="lazy" onerror="this.style.display=\'none\';this.nextSibling.style.display=\'flex\'">'
           + '<div style="display:none;width:48px;height:48px;margin:0 auto 10px;border-radius:10px;align-items:center;justify-content:center;background:rgba(73,232,194,.08)">'
           + getWalletLogo(id) + '</div>';
    }
    return '<div style="width:48px;height:48px;margin:0 auto 10px;border-radius:10px;display:flex;align-items:center;justify-content:center;background:rgba(73,232,194,.08)">'
         + getWalletLogo(id) + '</div>';
  }

  // Extension wallets — call native selWallet() which handles TN12 mnemonic/wasm signing
  var EXT_WALLETS = [
  ...
  // Mobile/hardware wallets — address paste only (no mnemonic, no extension)
  var ADDR_WALLETS = [
  ...
  function buildExtCard(w) {
    ...
    return [
      ...
      ' onclick="window._wpcConnect(\'', w.id, '\')"',
      ...
      'transition:border-color .2s,transform .2s,box-shadow .2s;position:relative;overflow:hidden;">',
        ...
        '</div>',
        '<button style="width:100%;padding:7px 4px;background:', found ? '#49e8c2' : 'rgba(73,232,194,.06)',
  ...
  function buildAddrCard(w) {
    var iid = 'wpc-addr-' + w.id.toLowerCase();
    return [
      ...
      'border-radius:14px;padding:18px 10px 14px;text-align:center;">',
        ...
        '<input id="', iid, '" type="text" placeholder="Paste kaspa:addr…"',
        ' style="width:100%;box-sizing:border-box;padding:8px 10px;',
        'background:rgba(8,13,26,.9);border:1px solid rgba(73,232,194,.18);border-radius:9px;',
        'color:#e2e8f0;font-size:10px;font-family:ui-monospace,monospace;outline:none;',
        'margin-bottom:8px;transition:border-color .18s"/>',
        '<button onclick="window._wpcAddrConnect(\'', w.id, '\',\'', iid, '\')"',
        ' style="width:100%;padding:7px 4px;background:rgba(73,232,194,.07);color:#49e8c2;',
        'border:1px solid rgba(73,232,194,.28);border-radius:8px;cursor:pointer;',
        'font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;margin-bottom:10px">',
        'Use Address',
        '</button>',
  ...
  // ── Extension connect: call native selWallet() first, fallback to provider API ──
  window._wpcConnect = function(id) {
    var w = EXT_WALLETS.find(function(x){ return x.id === id; });
    if (!w) return;

    // Not installed → open Chrome Web Store
    if (!w.detect()) {
      window.open(w.installUrl, '_blank');
      if (window.showToast) window.showToast('Opening ' + w.label + ' install page…', 'info');
      return;
    }

    // ALWAYS call native selWallet() — it handles TN12, mnemonic-mode, wasm signing etc.
    if (typeof window.selWallet === 'function') {
      window.selWallet(id);
      return;
    }

    // Fallback if selWallet not available yet
    var providers = {
      KasWare:  window.kasware,
      Kastle:   window.kastle,
      Kasperia: window.kasperia,
      OKX:      (window.okxwallet && window.okxwallet.kaspa) ? window.okxwallet.kaspa : null
    };
    var provider = providers[id];
    if (!provider) { if (window.showToast) window.showToast('Could not find ' + id + ' provider', 'error'); return; }
    var req = provider.requestAccounts || provider.connect;
    if (!req) { if (window.showToast) window.showToast(id + ' does not expose requestAccounts', 'error'); return; }
    req.call(provider).then(function(accounts) {
      var addr = Array.isArray(accounts) ? accounts[0] : (accounts && accounts.address ? accounts.address : accounts);
      if (!addr) { if (window.showToast) window.showToast('No address returned from ' + id, 'warn'); return; }
      window.walletAddress = window.htpAddress = window.connectedAddress = window.htpWalletAddress = addr;
      window.walletProvider = id; window.conn = true;
      window.dispatchEvent(new CustomEvent('htp:wallet:connected', { detail: { address: addr, wallet: id } }));
      window.dispatchEvent(new CustomEvent('kaspa:wallet:connected', { detail: { address: addr, wallet: id } }));
      if (window.showToast) window.showToast(id + ' connected ✓', 'success');
      if (window.updateWalletUI) window.updateWalletUI(id, addr);
      if (window.onWalletConnected) window.onWalletConnected(addr, id);
    }).catch(function(e) {
      if (window.showToast) window.showToast('Connection refused: ' + ((e && e.message) || e), 'error');
    });
  };

  // ── Mobile/hardware: paste kaspa: address ──────────────────────────────────
    ...
    return [
      '<div style="margin-bottom:20px">',
        '<div style="font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;',
          ...
          '<span style="color:#556070">Full connect</span>',
        '</div>',
      ...
      '<div>',
        '<div style="font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;',
          ...
          '<span style="color:#556070">Paste address</span>',
        '</div>',
  ...
  function run() {
    ...
    // ── Find or create the replacement target ──────────────────────────────
    // We target .w-grid (the old vertical wallet list) and replace it in-place.
    // Everything else (.w-sec blocks — mnemonic, hex, custom node) is left untouched.
    var oldGrid = section.querySelector('.w-grid');

    if (document.getElementById('wpc-wrap')) {
      // Already injected — just re-render the grid cards (detect states may have changed)
      var wrap = document.getElementById('wpc-wrap');
    ...
    if (oldGrid) {
      // Replace the old .w-grid with our new grid, keeping everything else
      oldGrid.parentNode.replaceChild(wrap, oldGrid);
    } else {
      // Fallback: prepend inside .mx container after .sh header
      var mx = section.querySelector('.mx') || section;
      var sh = mx.querySelector('.sh');
      if (sh && sh.nextSibling) {
        mx.insertBefore(wrap, sh.nextSibling);
      } else {
        mx.insertBefore(wrap, mx.firstChild);
      }
    }

    // Update the section header text
    var sh2 = section.querySelector('.sh h2');
    var sp  = section.querySelector('.sh p');
    if (sh2) sh2.textContent = 'Connect Wallet';
    if (sp)  sp.textContent  = 'Browser extensions connect instantly. For mobile/hardware, paste your KAS address. Use Mnemonic or Hex Key below to connect on TN12.';

    injectStyles();
    console.log('[HTP] wallet grid v7.2 injected — mnemonic/hex sections preserved');
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', run);
  else run();

  // Re-run when navigating to wallet view
