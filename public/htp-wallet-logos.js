...
(function applyNavFix() {
  function fix() {
    document.querySelectorAll('.nav-btn').forEach(function(btn) {
      var v = btn.getAttribute('data-v') || btn.getAttribute('onclick') || '';
      var isAct = btn.classList.contains('act');
      if (!isAct) return;
      var after = btn.querySelector('.nav-active-bar');
      if (!after) {
...
(function patchWalletGrid() {
  ...
  window._wpcConnect = function(id) {
    ...
    // Not detected → open the direct Chrome Web Store install page
    if (!w.detect()) {
    ...
    // Detected → try the app's native selWallet first
    if (typeof window.selWallet === 'function') { window.selWallet(id); return; }

    // Fallback: call requestAccounts directly
    var providers = {
    ...
    if (!req) { if (window.showToast) window.showToast(id + ' does not support requestAccounts', 'error'); return; }
    req.call(provider).then(function(accounts) {
      ...
      if (window.htpWalletV3 && window.htpWalletV3._onConnected) window.htpWalletV3._onConnected(addr, id);
    }).catch(function(e) {
      var msg = (e && e.message) ? e.message : String(e);
      if (window.showToast) window.showToast('Connection refused: ' + msg, 'error');
    });
  };
  ...
  // Re-run when user navigates to wallet view
