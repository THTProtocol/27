  ...
  function reg(el) {
    ...
    window.htpRegOracle = async function() {
      ...
      try {
        ...
        var d = await r.json();
        if (d.id) res.innerHTML = "<p style=\"color:#ffffff\">\u2713 Oracle registered: "+(d.address||"").slice(0,14)+"\u2026</p>";
  ...
  function att(el) {
    ...
    window.htpSubmitAtt = async function() {
      ...
      try {
        ...
        var d = await r.json();
        if (d.id || d.status === "attested") res.innerHTML = "<p style=\"color:#ffffff\">\u2713 Attestation submitted.</p>";
        else res.innerHTML = "<p style=\"color:#ff4444\">"+(d.error||JSON.stringify(d))+"</p>";
      } catch(e) { res.innerHTML = "<p style=\"color:#ff4444\">Request failed.</p>"; }
    };
  }
